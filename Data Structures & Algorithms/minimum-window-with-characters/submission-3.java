class Solution {
    public String minWindow(String s, String t) {
        ...
        for (int end=0; end<s.length(); end++) {
            ...
            while (doCountMatches(mapS, mapT)) {
                ...
                // if (countMatches(mapS, mapT) == totalMatches) {
    ...
    public int countMatches(Map<Character, Integer> mapS, Map<Character, Integer> mapT) {
        int matches=0;

        for (char k: mapT.keySet()) {
            if (mapS.containsKey(k) && mapS.get(k) == mapT.get(k)) matches++;
        }
